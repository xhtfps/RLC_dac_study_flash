...
void LCD_Draw_Picture(u16 xstr,u16 ystr,u16 xend,u16 yend,const u8 *pic)	    //画图
{
	u8 temp,t1;
	u16 x_t=xstr,y_t=ystr;//,x=xstr,y=ystr;
	const u8 *pusMsk = pic+6;//480000 65536
	for(y_t=ystr;y_t<yend;)
    {	
			temp = *pusMsk;
			for(t1=0;t1<8;t1++)
			{
				if(temp&0x80)OS_Point_Draw(x_t,y_t,BLACK);//画实心点
				else OS_Point_Draw(x_t,y_t,WHITE);   //画空白点（使用背景色）
				temp<<=1;
				x_t++;
				if(x_t==xend)
				{
...
void Draw_Graph(GRAPH_Struct *griddata,u8 yselect)
{
    ...
    if(yselect & LEFTY)
    {
        ...
        tempx1=x1;
        tempy1=y2-(griddata->left_buff[1]-griddata->left_ymin)/vpp*high;        // ADS1256测得的第一个值不准，所以从第二个开始
        ...
        for(i=1+1; i<length; i++)     // 因为前面从第二个值开始，所以此处也加1
        {
            tempx2=x1+i*xstep;
...
void Show_Label(GRAPH_Struct griddata,u8 yselect)
{
    ...
    step = (griddata.xmax - griddata.xmin) / (xnumber - 1);
    //step=(log10f(griddata.xmax)-log10f(griddata.xmin))/(xnumber-1);
	
    for(i=0; i<xnumber; i++) //横坐标轴单位长度标签
				OS_Num_Show(x1+i*xcase,y2,16,1, griddata.xmax / pow(10,xnumber - 1 - i) ,"%.0f");
        //OS_Num_Show(x1+i*xcase,y2,16,1, griddata.xmin + i * step,"%.0f");
        //OS_Num_Show(x1+i*xcase-20,y2+10,16,1, griddata.xmin + i * step,"%.0f"); //griddata.xmin +
        //OS_Num_Show(x1+i*xcase-20,y2+10,16,1, powf(10, ( log10f(griddata.xmin)+i*step )),"%.0f"); //griddata.xmin +

    if(yselect & LEFTY)
    {
        OS_String_Show(x1,y1-25,16,0,"20log|Av|/dB");//左边纵坐标轴标签

        OS_Line_Draw(x2+60,y1,x2+100,y1,griddata.left_ycolor);		//备注不同颜色曲线代表的含义
        OS_String_Show(x2-100,y1-25,16,0,"20log|Av|--Fre");
