...
extern uint32_t ADCData[ADCDataLength];
...
void Get_DCVol( float vol[2] )
{
	...
	{
		temp_vol1 = (float)( ADCData[count] & 0x0000ffff ) / 0xfff * BaseVol;
...
{ 
    ...
    // 添加检查：如果fs太小或为0，使用最小有效值
    if (fs <= 0) {
        fs = 1000;  // 默认1kHz采样率
    }
    
    // 添加上限检查，防止psc溢出
    if (fs > 1000000) {  // 最大1MHz
        fs = 1000000;
    }
    ...
    { 
        arr = 84000000 / (fs * psc);           // 关键行！
        psc ++; 
    } 
    while( arr > 65535 && psc < 65535 );      // 添加psc上限防止死循环
    
    if (psc >= 65535) {                        // 如果psc达到上限，使用安全值
        psc = 65534;
        arr = 65535;
    }
